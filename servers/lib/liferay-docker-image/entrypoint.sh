...
restore_database() {
  ...
  echo "CREATE DATABASE IF NOT EXISTS liferay CHARACTER SET utf8 COLLATE utf8_unicode_ci;" > $LIFERAY_CONFIG_DIR/dbscript.sql
  echo "GRANT ALL PRIVILEGES ON liferay.* TO '$DB_USERNAME'@'%';" >>$LIFERAY_CONFIG_DIR/dbscript.sql
  echo "FLUSH PRIVILEGES;" >> $LIFERAY_CONFIG_DIR/dbscript.sql
  echo "USE liferay;" >> $LIFERAY_CONFIG_DIR/dbscript.sql
  cat $LIFERAY_CONFIG_DIR/database.sql >> $LIFERAY_CONFIG_DIR/dbscript.sql

  mysql -h $DB_HOST -u $DB_USERNAME -p$DB_PASSWORD < $LIFERAY_CONFIG_DIR/dbscript.sql
}
...
prepare_liferay_portal_properties() {
  if [[ ! -f "$LIFERAY_CONFIG_DIR/portal-ext.properties" ]]; then
    echo "No '$LIFERAY_CONFIG_DIR/portal-ext.properties' file found.
